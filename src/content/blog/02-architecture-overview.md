...
- Vercel Hobby の商用利用ライン: **広告掲載を含むデプロイは商用扱い、AdSense は明示的に商用例**（2026-05-08 公式 Fair Use Guidelines で確認済）。LIFF 本体側を Hobby に置き続けることの是非は規約の解釈次第のグレーゾーンで、本気でやるなら Vercel サポートへ要問い合わせ
- Firestore 無料枠の各上限は 2026 年時点の値を公式で確認（読み取り 50K / 書き込み 20K / 1GiB は古い数字の可能性）
- Cloud Run functions（旧 第 2 世代 Functions）の無料枠: **月 200 万リクエスト + 180,000 vCPU-秒 + 360,000 GiB-秒（リクエストベース、us-central1）**（2026-05-08 確認済）。第 2 世代は 2024 年 8 月に Cloud Run functions に改称、Cloud Run と統合
- Cloudflare Pages の無料枠（帯域・リクエスト無制限、ビルド 500 回 / 月）の最新値は要確認
- LINE Messaging API の Push 月間上限: **コミュニケーションプラン（フリープラン）= 月 200 通で確定**（2026-05 確認済）。スタンダードプラン超過分の追加メッセージ料金は 2026 年 10 月に改定予定なので、公開タイミングによっては要再チェック

### ユーザー追記推奨箇所

- 自分の予測 DAU と無料枠の乖離分析を表で
- 構成図のスクショ（できれば draw.io / Excalidraw 等で清書）
- 想定外コストが出た事例があれば追記

### 引用元

- `vaccine-note-line-docs/ARCHITECTURE.md`
- `vaccine-note-line-docs/RESUME.md` 確定方針
- 想定文字数: 2,000〜2,500 字 → 本文約 2,800 字
