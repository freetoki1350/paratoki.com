...
- [#11 LINE Developers Console の「LINE ログインチャネル」と「Messaging API チャネル」を取り違えた末路](./11-line-channel-mixup.md)
...
- お名前.com の画面項目名・遷移パスは 2026 年 5 月時点でスクショ撮り直し（リニューアル多発）
- Hobby プランで CNAME 接続自体は可能（2026-05 確認済）。ただし広告を含むサイトは Vercel 規約上商用扱いになるため、AdSense を貼る予定があるなら Pro プランまたは別ホスティングを検討
- DNS 反映時間: 20 〜 30 分（2026-05-08 著者確認）

### ユーザー追記推奨箇所

- お名前.com / Vercel の各画面のスクショ
- 自分が踏んだ DNS の TTL / 反映時間の実数値

### 引用元

- `vaccine-note-line-docs/DEPLOY.md` セクション 7
- 想定文字数: 1,500〜2,000 字 → 本文約 2,400 字
