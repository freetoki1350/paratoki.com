...
- 消費時間: 30 分（2026-05-08 著者確認）
- 2025 年のフロー変更に関する一次ソース（LINE 公式のリリースノート URL）を追加すると説得力が増す
- 「ID トークン検証エンドポイントが Messaging API 側 ID で 400 系」のステータスコード正確値は実機ログ確認

### ユーザー追記推奨箇所

- 401 を眺めていた時の心理（共感ポイント）を 1 文
- 自分が最終的に使った「シークレット命名規則」があれば差分追記
- コンソール画面のスクショ（チャネル ID の見つけ方）

### 引用元

- `vaccine-note-line/functions/src/verifyLineIdToken.ts`（401 を返す箇所）
- `vaccine-note-line-docs/DEPLOY.md` 2-2〜2-6
- 想定文字数: 1500〜2000 字 → 本文約 2,500 字（やや超過、推敲時に削る余地あり）
