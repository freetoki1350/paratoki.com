...
- 児童関連法令: **2026 年 1 月 9 日公表の「3 年ごと見直し」制度改正方針** で 16 歳未満を「こども」と定義し法定代理人同意を義務化する方向（2026-05-08 確認済）。改正法案は 2026 年通常国会提出予定、成立後は令和 8 年改正個人情報保護法。雛形「13 歳未満」表現は古く、本記事では年齢数字を確定させない柔らかい表現に修正済
- AdSense ポリシー（2026 年版）: 健康トピックは **センシティブカテゴリ** で、診断・治癒・確実効果を示唆する表現や未承認治療の宣伝が禁止。コンテンツ品質要件も 2026 年に厳格化、disclaimer（"This site is not a substitute for professional medical/legal/financial advice" 等）の設置が推奨されている（2026-05-08 確認済）。本記事ではこの内容を本文に反映済
- 「弁護士確認推奨」は本気で利用者にかけたい場合は連絡先を併記したほうが親切
- ドメイン・屋号は example.com / Example Lab に汎用化済（公開時に各自の値で置換する想定）

### ユーザー追記推奨箇所

- 自分の屋号・メアド設定の経緯
- 実際の文面（伏せながらでも）
- AdSense 審査でプライバシーポリシーが理由で否認・通過した経験

### 引用元

- `vaccine-note-line-docs/PRIVACY_POLICY.md`
- `vaccine-note-line-docs/TERMS_OF_SERVICE.md`
- 想定文字数: 2,000〜2,500 字 → 本文約 2,500 字
